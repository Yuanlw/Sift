...
create table sift_gateway_tokens (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references users(id) on delete cascade,
  token_hash text not null unique,
...
create table sift_gateway_usage_ledger (
  id uuid primary key default gen_random_uuid(),
...
create index sift_gateway_tokens_user_status_idx on sift_gateway_tokens (user_id, status, created_at desc);
create index sift_gateway_tokens_prefix_idx on sift_gateway_tokens (token_prefix);
create index sift_gateway_usage_user_period_idx on sift_gateway_usage_ledger (user_id, period_start, period_end, created_at desc);
create index sift_gateway_usage_token_created_idx on sift_gateway_usage_ledger (token_id, created_at desc);
create index manual_refunds_user_created_idx on manual_refunds (user_id, created_at desc);
create index manual_refunds_status_created_idx on manual_refunds (status, created_at desc);
create index support_case_notes_user_created_idx on support_case_notes (user_id, created_at desc);
create index support_case_notes_issue_created_idx on support_case_notes (issue_type, created_at desc);
create index product_events_user_event_idx on product_events (user_id, event_name, occurred_at desc);
create index product_events_event_time_idx on product_events (event_name, occurred_at desc);
